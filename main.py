...
class Perceptron:
    ...
    def __init__(self, num_iter=10, const=0.1, verbose=False, bias=True):
        self.const = const
    ...
    def train(self, X, y, weight=None):
        ...
        for iterr in range(self.num_iter):
            ...
            for index in tqdm(range(input_count)):
                ...
                if self.bias: self.__bias += update * 1
    ...
    def predict(self, X):
        return self.__activation(np.dot(X, self.w) + self.__bias)

    def __activation(self, x):
        return np.where(x >= 0.0, 1, -1)

def main():
    unit = Perceptron(num_iter=10, const= 0.1, verbose=True, bias=False)
